...
function FuturesChartPage() {
  ...
  useEffect(() => { if (data[replayIndex]) setReplayDate(data[replayIndex].time) }, [data, replayIndex])
  const chartData = useMemo(() => replayMode ? data.slice(0, replayIndex + 1) : data, [data, replayIndex, replayMode])
  useEffect(() => {
  ...
  const stepReplay = (amount) => { if (!data.length) return; setReplayMode(true); setPlaying(false); setReplayIndex((current) => Math.max(0, Math.min(data.length - 1, current + amount))) }
  const toggleReplay = () => { if (!replayMode) { const start = replayIndex >= data.length - 1 ? Math.max(0, data.length - 60) : replayIndex; setReplayIndex(start); setReplayDate(data[start]?.time || ''); setReplayMode(true) } else { setPlaying(false); setReplayMode(false) } }
  ...
  return <section className="chart-hero">
    ...
    {replayMode && <div className="replay-bar"><button type="button" title="Previous bar" onClick={() => stepReplay(-1)}>◀</button><button type="button" className="replay-play" title={playing ? 'Pause replay' : 'Play replay'} onClick={() => setPlaying((active) => !active)}>{playing ? 'Ⅱ' : '▶'}</button><button type="button" title="Next bar" onClick={() => stepReplay(1)}>▶</button><span className="replay-label">BAR {replayIndex + 1} / {data.length}</span><input className="replay-scrubber" type="range" min="0" max={Math.max(0, data.length - 1)} value={Math.min(replayIndex, Math.max(0, data.length - 1))} onChange={(event) => { setReplayIndex(Number(event.target.value)); setReplayDate(data[Number(event.target.value)]?.time || '') }} /><span className="replay-date">{replayDate || 'Select a bar'}</span><label className="replay-speed">Speed<select value={speed} onChange={(event) => setSpeed(event.target.value)}><option value="0.5">0.5×</option><option value="1">1×</option><option value="2">2×</option><option value="4">4×</option></select></label><span className="shortcut-hint">Space play · ← → step · R exit</span></div>}
